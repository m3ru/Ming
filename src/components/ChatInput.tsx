  ...
  useEffect(() => {
    window.fetch = async (url, options) => {
      if (url === "/api/save-chat" && options.method === "POST") {
        ...
        return new Response(
          JSON.stringify({ status: "ok", message: "Chat saved successfully." }),
          {
    ...
    >
      ...
      <div className="p-8">
        ...
        <p className="mt-4 text-gray-600">
          This is the main content of your page. The chat exporter and panel
          will appear around it.
        </p>
        <p className="mt-2 text-gray-600">
          Click the "Export Chat" button in the top right to post the chat log
          to the (mock) backend.
        </p>
      </div>
    </ChatDataExporter>
